	...
	return [
		{
			...
			content: [
				...
				"",
				"Worked example — “summarise yesterday's email”. Note that reading mail is a",
				"tool on an `agent` node, not a node type, and that this is how nearly every",
				"outside capability is expressed:",
				'{"nodes":{"begin":{"type":"begin","label":"Start","params":{},"upstream":[],"downstream":["fetch"]},"fetch":{"type":"agent","label":"Read the inbox","params":{"prompt":"Find the messages from the last day and list sender, subject and gist.","tools":["gmail_search"]},"upstream":["begin"],"downstream":["report"]},"report":{"type":"llm","label":"Summarise","params":{"prompt":"Summarise these and say which matter:\n{{fetch.text}}"},"upstream":["fetch"],"downstream":[]}}}',
				"",
				...
				"- A node that works on an earlier node's output MUST reference it. A prompt that",
				"  merely describes the data — \"These are the important emails:\" — receives nothing,",
				"  and the model answers that it has no access to any email. Write",
				"  \"Summarise these:\n{{read_inbox.text}}\" instead.",
				...
				"- Never use a node type or a tool that is not listed above. If one seems to be missing, look again at the tool list: almost everything outside the flow is a tool on an `agent` node.",
				"",
				"Refusing is almost always the wrong answer. Draft the closest working flow",
				"instead, and leave a node's `params` empty where the detail is unclear. Refuse",
				'only if no combination of the lists above comes close, and then answer {"error":"..."}',
				"naming the specific capability you needed — the tool or node you looked for and",
				"could not find. Do not answer with a general statement that something is missing.",
			].join("\n"),
		},
		{
			role: "user" as const,
